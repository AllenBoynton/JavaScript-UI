...
NSString * k1506_InfoApp_Boynton_Allen$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1506_InfoApp_Boynton_AllenObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1506_infoapp_boynton_allen);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1506_infoapp_boynton_allen gc];
}
...
{
	// create k1506_InfoApp_Boynton_Allen global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1506_InfoApp_Boynton_Allen" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1506_infoapp_boynton_allen = [[k1506_InfoApp_Boynton_AllenObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1506_infoapp_boynton_allen];
	
	NSString *_k1506_infoapp_boynton_allenNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1506_infoapp_boynton_allenNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1506_infoapp_boynton_allen valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1506_InfoApp_Boynton_Allen$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1506_infoapp_boynton_allen.kroll" 
